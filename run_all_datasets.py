...
def run_single_dataset(dataset_path, subdirs, script_path, timeout):
    ...
    if dataset_name.lower() in ['cifar', 'cifar-10', 'cifar10']:
        ...
        effective_timeout = max(timeout, 14400)
        num_images = '3'
...
def create_summary_tables(df, subdirs, run_dir):
    """
    Create CSV and LaTeX summary tables
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch process all tabular datasets with CVAE'
    )
    ...
    print(f"Configuration: ADOPT Optimizer")
    print(f"{'='*70}")
