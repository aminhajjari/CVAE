...
class CVAEWithTabEmbedding(nn.Module):
    def __init__(self, input_dim, tab_latent_size, num_classes, latent_size=8, vif_values=None):
        ...
        # Encoder: image + tabular → shared representation
        ...
        # Decoder: latent + tabular → reconstructed image
        ...
        # Optional classifiers from latent
    ...
    def forward(self, img_x, tab_x):
        # img_x: real image flattened, tab_x: tabular
...
optimizer = ADOPT(
    cvae.parameters(),
    lr=1e-3,                # start with 1e-3 as in the ADOPT examples
    betas=(0.9, 0.9999),    # recommended in the paper
    eps=1e-6,               # slightly larger eps than typical AdamW
    weight_decay=1e-4,      # same as your AdamW
    decouple=True,          # because you previously used AdamW
)
...
def loss_function(recon_x, x, tab_pred, tab_labels, img_pred, img_labels, mu, logvar, beta=1.0):
    # Reconstruction (MSE; use BCE if images are in [0,1])
...
def train(model, train_data_loader, optimizer, epoch):
    ...
    for tab_data, tab_label, img_data, img_label in train_data_loader:
        img_data = img_data.view(-1, 28*28).to(DEVICE)   # real images
        tab_data = tab_data.to(DEVICE)
        img_label = img_label.to(DEVICE).long()
        tab_label = tab_label.to(DEVICE).long()

        optimizer.zero_grad()
        # Feed real image to the encoder, not random noise
